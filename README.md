...
| `ship-out` | Build a unit from a goal + the spec → reconcile → verify → optionally commit/PR (never merges) | **`/ship-out`** (user) |
...
- **Verbs — user-invoked only (`disable-model-invocation: true`):** `spec-out` (deliberate discovery) and `ship-out` (deliberate build + outward delivery). Neither auto-fires.
...
| **Build & deliver** | `ship-out` | you | a goal + the spec → an implementation, reconciled spec, gated PR |
| *(coherence)* | `audit` | guard (auto) | code↔spec divergence → human picks the fix |

Ad-hoc work needs neither verb — just code; `record` and `audit` fire during any work to keep the spec coherent. `ship-out` is for the deliberate "build this unit properly and deliver it" move.
